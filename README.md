...
The server starts both the TUI and the web console by default. The web console opens in the default browser at the root
URL. Use CLI overrides when needed:

```text
host-bridge-mcp --host 127.0.0.1 --port 8810 --tui false --web true
```

`--tui` and `--web` accept an optional boolean value, so both `--tui false` and `--tui=false` are valid. When `--tui
false` is used, logs are written as normal tracing output to the terminal and confirmation-required commands are
rejected because no interactive approver is available. The command-line help and version output are generated by the CLI
parser.
...
`{"decision":"approve-once"}`, `{"decision":"approve-for-session"}`, or `{"decision":"reject"}`.
