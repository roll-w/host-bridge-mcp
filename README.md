...
| `logging`   | In-memory log retention and log-file behavior.                                |

### `server`

| Key                         | Default          | Description                                                                                                   |
|-----------------------------|------------------|---------------------------------------------------------------------------------------------------------------|
| `server.address`            | `127.0.0.1:8787` | HTTP listen address. Keep it on loopback unless the network is trusted.                                       |
...
| Key                           | Default | Description                                                                                     |
|-------------------------------|---------|-------------------------------------------------------------------------------------------------|
| `logging.memory-buffer-lines` | `2000`  | Number of recent log lines kept for the TUI; must be greater than zero.                         |
| `logging.file-path`           | unset   | Explicit log-file path. When unset, the server chooses its data-directory path.                 |
| `logging.persist-file`        | `false` | Keep the log file after shutdown. Persistent logs are appended and previous files are archived. |
...
Policy actions are configured under `execution.commands`:
...
to approve once, `s` to approve the same request for the current MCP session, or `r` to reject it. A session approval is
an in-memory grant bound to the MCP session and the execution context: target, platform, command line, executable,
arguments, working directory, environment, and shell-operator flag. The request timeout and output presentation limits
are evaluated separately and do not change the session approval. It does not approve a wildcard command family and is
cleared when the server shuts down. There is currently no Agent auto-approval API.
...
Requires a stable Rust toolchain and Cargo.
